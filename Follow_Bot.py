...
class Follow:
    def __init__(self, username, account, proxy) -> None:
        self.proxy = proxy
        self.user_agent = UserAgent().random
        self.session = requests.Session()
        self.userid = self.get_id()
    ...
    def get_id(self):
        self.create_session()
        get_data = self.session.get(f"https://www.instagram.com/web/search/topsearch/?context=blended&query={self.follow_target}&rank_token=0.3953592318270893&count=1").json()
        try:
            return str(get_data).split("'user': {'pk': '")[1].split("', 'username'")[0]
    ...
    def __main__(self):
        self.create_session()
        send_req = self.session.post(f'https://www.instagram.com/web/friendships/{self.userid}/follow/')
        if send_req.json()['status'] == 'ok':
            sys.stdout.write(f'{Fore.GREEN}[{Fore.WHITE}+{Fore.GREEN}]{Fore.WHITE} Sent on [{self.username}:{self.password}]\n')
            return True
        else:
            sys.stdout.write(f'{Fore.RED}[{Fore.WHITE}-{Fore.RED}]{Fore.WHITE} Could not send on [{self.username}:{self.password}]\n')
            return False 
...
if __name__ == '__main__':
    ...
    target = input('What account do you want to follow -> ')

    for account in accounts:
        threading.Thread(target=Follow(target, account, random.choice(proxies)).__main__)
