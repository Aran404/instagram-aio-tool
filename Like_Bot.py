...
class Likes:    
    def __init__(self, post, account, proxy) -> None:
        self.post = post
        self.proxy = proxy
    ...
    def __main__(self):
        like_post = self.session.post(f'https://www.instagram.com/web/likes/{self.post_id}/like/')
        if like_post.json()['status'] == 'ok':
            sys.stdout.write(f'{Fore.GREEN}[{Fore.WHITE}+{Fore.GREEN}]{Fore.WHITE} Liked on [{self.username}:{self.password}]\n')
            return True
        else:
            sys.stdout.write(f'{Fore.RED}[{Fore.WHITE}-{Fore.RED}]{Fore.WHITE} Could not like on [{self.username}:{self.password}]\n')
            return False 
...
if __name__ == '__main__':
    ...
    post = input('What post do you want to like -> ')

    for account in accounts:
        threading.Thread(target=Likes(post, account, random.choice(proxies)).__main__)
